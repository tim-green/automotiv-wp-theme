...
if (!function_exists('custom_shipping_option')) {
    function custom_shipping_option($rates){
        ...
        // unset rates if correct rate for shipping is available
        if ( isset( $rates['flexible_shipping_1_2'] ) ) {
            unset( $rates['flexible_shipping_auspost:0:GROUND_HOME_DELIVERY'] );
        }  
...
function on_action_cart_updated()
{
    if( is_cart() || is_checkout() && !is_wc_endpoint_url( 'order-pay' ) ) {
        ...
        if( $total < $min_total  ) {
            ...
            <strong>$35.00 required</strong>.</div>';
    ...
    <strong>$12.50 shipping</strong>!
    </div>';
        } elseif ($current_amount >= 150) {
            echo '<div class="alert alert-success mt-3" role="alert">
        Congratulations your order can be shipped for only <strong>$12.50</strong>!
    </div>';
        }
    }
}
